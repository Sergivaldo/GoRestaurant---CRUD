...
function adicionar_prato(){
  if(sao_campos_validos()){
    const prato = {
      link: document.getElementById('link_imagem').value == ''? 'imagem alternativa': document.getElementById('link_imagem').value,
...
function criar_card(prato,index){
  ...
  `
  card.querySelector('.card-food__img').style.backgroundImage = `url(${prato.link})`
